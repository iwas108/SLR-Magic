...
import { fetchHistory, deleteHistoryItem, bulkDeleteHistory, clearHistory, fetchEndpoints } from '../services/api';
import { ChevronLeft, ChevronRight, Copy, X, Trash2, Search, ArrowUp, ArrowDown } from 'lucide-react';
...
const History = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            setLoading(true);
            const [historyData, endpointsData] = await Promise.all([
                fetchHistory({
                    page,
                    limit,
                    search,
    ...
    }, [page, limit, search, endpointFilter, sortBy, sortDesc, timeStart, timeEnd]);
    ...
    const handleClearAll = async () => {
        if (window.confirm('WARNING: Are you sure you want to completely clear ALL history records? This cannot be undone.')) {
            try {
                await clearHistory();
                setSelectedIds([]);
                setPage(1);
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            {selectedIds.length > 0 && (
                <div className="mb-4 flex items-center bg-blue-50 p-3 rounded-md">
                    <span className="text-blue-800 mr-4 font-medium">{selectedIds.length} items selected</span>
                    <button
