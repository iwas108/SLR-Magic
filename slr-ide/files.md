...
| `package.json` | Dependency / Scripts | Defines NPM package dependencies, project metadata, and execution scripts (e.g., `dev`, `prebuild`, `build`, `typecheck`, `lint`). |
...
| `public/fonts/` | Offline Assets / Typography | Self-hosted, 100% offline publication font suite (WOFF2/TTF assets for Computer Modern, STIX Two Text, Carlito, EB Garamond, and Roboto) ensuring zero CDN leakage and airgap compliance. |
...
| File Path | Architectural Layer | Function & Purpose |
| :--- | :--- | :--- |
| `scripts/dev.mjs` | Server Launcher | Development server launcher resolving network interfaces, custom port configuration, and running Next.js dev server with Turbopack high-speed engine by default. |
| `scripts/start.mjs` | Server Launcher | Production server launcher binding Next.js server to resolved network configurations. |
...
| `scripts/surgical_migrate_rq3a_mcu.py` | Database Migration | Calibrates RQ3a edge hardware taxonomy for active project, renames category to 'Microcontrollers (MCUs)', reallocates pure application processors to IPCs/FPGAs, and asserts 14 MCU papers (30.4%) cohort invariant. |
...
| `lib/services/taxonomy-resolver.ts` | Backend / Domain Service | Centralized Taxonomy Resolver service enforcing exact case-insensitive key matching, canonical string/dash normalization, isomorphic stage dominance resolution (handling both SQLite strings and parsed objects), and single-source QA/extracted data retrieval. |
...
| `components/features/pre-calibration/BlindedAdjudicationPanel.tsx` | Presentation Component | Comprehensive blinded review adjudication component visualizing agreement metrics (Kappa, Observed Agreement, Expected Chance, Precision, Weighted Kappa, Schema Match) across Pools A, B, and C, paired with an interactive Pre-Calibration Adjudication Explorer featuring pool switching tabs, sub-tab toggles (Cohort & Discrepancies vs Calibration Audit Ledger), search/filtering, and modal inspection triggers. |
| `components/features/pre-calibration/AdjudicationInspectionModal.tsx` | Modal Component | Comprehensive dual-pane inspection modal supporting Pools A, B, C, and Rolling Batches, rendering bibliographic metadata, full abstracts, side-by-side Reviewer Alpha vs Beta vs Consensus comparisons, and cryptographic commit stamps with keyboard navigation. |
...
| `components/features/insight-export/CohortPaperDetailsModal.tsx` | Modal Component | Comprehensive fullscreen paper inspection modal with split-screen view: local sourced PDF preview (in IDE) or cloud PDF sync (in viewer) on the left, and RQs with taxonomy mappings, extraction reasoning, verbatim quotes, QA appraisal scores, and raw JSON on the right. |
...
| `components/features/modals/ScientificRigorLlmModal.tsx` | Modal Component | Interactive modal for configuring and exporting comprehensive scientific rigor JSON datasets across 8 dimensions (AI technical specs, PRISMA flow, systematic search strategies, pre-calibration, prompt optimization, stage comparisons, rolling batch QC, and LLM narrative directives). |
| `components/features/modals/VisualizerModal.tsx` | Modal Facade / Orchestrator | High-level modal shell wrapping `VisualizerProvider`, `VisualizerHeader`, and `VisualizerStudio` with support for `allCohortPapers`, `projectId`, and autonomous DB hydration. |
...
| `components/features/modals/visualizer/types.ts` | Type Definitions | Domain interfaces for ChartType, LayoutMode, SlotId, SubfigureLabelStyle, SlotConfig, GlobalStyleConfig, ThemePreset (58 academic presets), ThemePalette (with tokens: accent, secondary, surface, gridLine, heatScale, isDark), FontFamily, MetricMode, SunburstLevelConfig, VisualizerPresetPayload, VisualizerModalProps, and BreakdownRow. |
| `components/features/modals/visualizer/constants/` | Constants | Modular constants for 19 scientific chart definitions (`chartTypes.ts`), multi-block layout definitions (`layoutPresets.ts`), 58 prestigious academic and domain palettes (`themePalettes.ts`), typography configs (`fontFamilies.ts`), and defaults (`defaultConfigs.ts`). |
...
| `components/features/modals/visualizer/utils/` | Pure Utilities | Pure mathematical, formatting, and export utilities: Hare-Hamilton 100.00% quota balancer (`quotaBalancer.ts`), multi-stop gradient shaders & academic color token resolvers (`colorUtils.ts`), multi-value & taxonomy extraction (`dataExtractor.ts`), and high-DPI multi-panel composite canvas & SVG rendering (`exportUtils.ts`). |
| `components/features/modals/visualizer/generators/` | Chart Strategy Generators | 7 specialized chart-family strategy modules with deep per-chart academic palette integration across all 19 chart types, direction-aware linear gradients, haloed markers, crisp slice borders, continuous heat scales, and master `buildChartOption()` dispatcher. |
| `public/fonts/` | Asset Directory | Bundled local offline TrueType and WOFF2 fonts for Computer Modern / Latin Modern Roman / CMU Serif typography for instant, zero-latency canvas rendering in LaTeX and Elsevier journal figures. |
...
| `components/features/modals/visualizer/hooks/useVisualizerWorkspace.ts` | Custom Hook | Manages fullscreen window sizing, live preview split mode, Zen theater mode, canvas backdrops, subfigure inspection, cohort scope selection (`full` vs `filtered`), and keyboard ergonomics. |
| `components/features/modals/visualizer/context/` | React Context | `VisualizerContext` and `VisualizerProvider` unifying all state hooks, direct SQLite DB hydration (`/api/insight/final-cohort`), `SYNC_PAPERS` multi-tab sync, and chart option generation to eliminate prop drilling. |
| `components/features/modals/visualizer/components/` | Step & Sub-Components | Focused step components (`Step1ChartSelector`, `Step2DataMapping`, `UniversalFineTunePanel`, `Step4PreviewStage`, `VisualizerHeader`) and subcomponents (`LayoutTemplateSelector`, `SlotSwitcherBar`, `LiveSplitPreview`, `CustomGroupingManager`, `BreakdownTablePanel`, `SunburstLevelConfigPanel`, `HorizontalBarConfigPanel`, `CameraControlsOverlay`, `ExportPanel`). |
...
| `components/features/modals/visualizer/components/subcomponents/SunburstLevelConfigPanel.tsx` | Subcomponent | Comprehensive fine-tuning panel for Sunburst and Hierarchical Ring charts featuring layout presets, color shading modes, auto-contrast text legibility, per-level typography, slice radius, borders, min angle thresholds, automatic overlap suppression, and dedicated chart legend placement controls. |
| `components/features/modals/visualizer/components/subcomponents/HorizontalBarScatterConfigPanel.tsx` | Subcomponent | 8-card dedicated publication fine-tuning suite for Dual-Axis Horizontal Bar & Scatter Overlay charts with unified series aesthetics, scatter markers, category sorting/overrides, categorical Y-axis layout & typography (width, word wrap on slashes, margin, anti-collision title gap), dual X-axes calibration, canvas grid collision clearance, and chart legend placement. |
| `components/features/modals/visualizer/components/subcomponents/ClusteredBarConfigPanel.tsx` | Subcomponent | Dedicated fine-tuning panel for Clustered / Comparative Bar Charts with controls for bar orientation, clustering gap, inner gap, value label typography, data labels toggle, and legend placement controls (`top`, `bottom`, `left`, `right`). |
| `components/features/modals/visualizer/components/subcomponents/StackedBarConfigPanel.tsx` | Subcomponent | Dedicated fine-tuning panel for Stacked Bar Charts providing layout switcher, 100% normalization, reversed stacking, segment sorting, segment data labels, total summit labels, per-series colors, target benchmark lines, and chart legend placement controls. |
| `components/features/modals/visualizer/components/subcomponents/UniversalFineTunePanel.tsx` | Subcomponent | Streamlined customization hub unifying a 58-preset Academic Color Palette selector with the active chart's dedicated configuration panel, suppressing redundant cartesian axis, legend, and margin duplicates for encapsulated chart types like Horizontal Bar Scatter. |
| `components/features/modals/visualizer/generators/categoricalBarGenerators.ts` | Chart Strategy Generator | Generates publication-grade ECharts options for Vertical Bar, Horizontal Bar, Stacked Bar, and Dual-Axis Horizontal Bar & Scatter charts with Hare-Hamilton 100.00% quota balancing, dynamic anti-collision clearance, percentage-to-pixel grid scaling, and synchronized decimal rounding. |
...
| `components/features/modals/visualizer/components/subcomponents/ChartConfigPanels.tsx` | Subcomponents | Centralized collection and exports of self-contained fine-tuning parameter inspector panels across all scientific chart types (including `VerticalBarConfigPanel`, `PieDonutConfigPanel`, `LineConfigPanel`, `HeatmapConfigPanel`, `FunnelConfigPanel`, `TreemapConfigPanel`, `BoxplotConfigPanel`, `ScatterConfigPanel`, `BubbleConfigPanel`, `RadarConfigPanel`, `GraphConfigPanel`, `GaugeConfigPanel`, and `CalendarConfigPanel`). |
| `components/features/modals/visualizer/generators/clusteredBarGenerators.ts` | Chart Strategy Generator | Generates publication-grade ECharts options for Clustered / Comparative Bar Charts with per-series color bindings, dynamic label contrast, and custom text color resolution. |
| `components/features/modals/visualizer/components/subcomponents/FieldAutocomplete.tsx` | Subcomponent | Searchable glassmorphic autocomplete combobox with keyboard navigation, grouped optgroups, live prevalence badges ($n/N$), RQ badges, and zero-hit integrity warning alerts. |
| `components/features/modals/visualizer/generators/axisConfigHelper.ts` | Chart Strategy Helper | Centralized scientific ECharts axis configuration builder providing standardized title typography, dynamic anti-collision title gaps, tick density, word-wrapping with slash delimiter preservation, number formatting, baseline borders, and gridlines across all Cartesian charts. |
| `components/features/modals/visualizer/components/subcomponents/ScientificAxisConfigPanel.tsx` | Subcomponent | 4-tab publication-grade scientific axis customization panel (`Titles`, `Ticks & Angles`, `Units & Formats`, `Grid & Ticks`) providing granular controls for X/Y labels, font weights, rotation presets, and density intervals. |
| `components/features/modals/visualizer/components/subcomponents/CrossTabMatrixPanel.tsx` | Subcomponent | Interactive 2D Cross-Tabulation Matrix and Flattened Long Table component with instant TSV clipboard copy, CSV export, and modal expansion triggers. |
| `components/features/modals/visualizer/components/subcomponents/CrossTabMatrixModal.tsx` | Subcomponent Modal | Fullscreen-capable expanded modal dialog for viewing, analyzing, copying, and exporting large cross-tabulation matrices and long tables. |
...
| `components/features/modals/visualizer/components/subcomponents/CollapsibleToolbox.tsx` | Subcomponent | Professional collapsible toolbox container inspired by Adobe Premiere Pro's Effect Controls / Lumetri panels, featuring disclosure chevron toggles, themed category icons, active summary chips, toggle switches, and parameter reset triggers. |
| `components/features/modals/visualizer/components/subcomponents/ChartLibraryModal.tsx` | Subcomponent Modal | Centralized FAIR-compliant Chart Library modal interfacing with SQLite `saved_charts` table for project-scoped chart persistence, studio hydration, duplicate, delete, and legacy JSON preset migration. |
...
| `components/features/insight-export/CohortPaperDetailsModal.tsx` | Modal Component | Comprehensive fullscreen paper inspection modal featuring split-screen local PDF iframe viewer (in IDE) or synced cloud PDF button (in viewer) alongside RQ extraction taxonomy mappings, verbatim evidence quotes, QA appraisal breakdown, and raw JSON payload. |
| `components/features/insight-export/FinalCohortPanel.tsx` | View Component | Authoritative final cohort table view providing multi-column search, dynamic column resizing, stage-dominant extraction parsing, and launch triggers for the Fullscreen Inspection Modal, Visualizer Studio, and LLM Context Builder. |
...
| `components/features/post-validation/ExportMappingsModal.tsx` | Modal Component | Standalone modal dialog for exporting Umbrellanizer taxonomy mappings and justifications in CSV/JSON formats with variable scoping. |
...
| `api/charts/route.ts` | REST Endpoint | Project-isolated REST endpoint for FAIR-compliant centralized database chart storage, supporting GET (list project charts), POST (create/update chart configurations with validation), and DELETE (scoped chart removal). |
...
| `api/insight/scientific-rigor/route.ts` | REST Endpoint | Aggregates and exports comprehensive scientific rigor JSON datasets (PRISMA, systematic search strategies, pre-calibration, prompt optimization, gold standard comparisons, and rolling batch validation) for LLM narrative context. |
...
| `api/umbrellanizer/export/route.ts` | REST Endpoint | Handles GET requests to export raw-to-umbrella category mappings and justifications in CSV or JSON format with multi-project isolation and occurrence metrics. |
...
| `scripts/dev.mjs` | Launcher Script | Node.js development server launcher that resolves host/port from file-based configuration and starts Next.js dev server with Webpack as stable default (opt-in Turbopack via `--turbo`) and formatted LAN URLs. |
...
| `components/features/modals/visualizer/generators/trendLineGenerators.ts` | Chart Strategy Generator | Implements dual-paradigm Line Chart generation supporting empirical literature synthesis trends and theoretical Epistemic Simulation & Trajectory models with exponential drift baselines, discrete recursive estimators with subtle linear alpha gradients (≤0.08) to eliminate print overlap, physical radio transmission event peak markers (▲), and benchmark mark lines. |
| `components/features/modals/visualizer/constants/fontFamilies.ts` | Typography Constants | Publication-grade font family definitions and CSS font-stack resolver supporting Elsevier journals, LaTeX Computer Modern, Times New Roman, Helvetica, Calibri, Georgia, Garamond, Roboto, and standard academic typefaces. |
| `components/features/modals/visualizer/generators/correlationGenerators.ts` | Chart Strategy Generator | Implements 2D Categorical Cross-Tabulation Matrix Bubble Chart and continuous 3D scatter chart generators with dynamic safe collision margins and multi-series category legend chips. |
| `components/features/modals/visualizer/components/subcomponents/CustomGroupingModal.tsx` | Modal Component | Standalone encapsulated modal dialog for the Universal Custom Grouping Layer Manager with preset management, stats-assisted tail grouping, and keyboard ergonomics. |
| `components/features/modals/visualizer/components/subcomponents/UniversalFineTunePanel.tsx` | UI Container | Streamlined visualizer customization panel mounting UniversalPaletteFontPanel as Card 1 (Academic Palette & Global Typography) and routing to dedicated per-chart fine-tuning panels, Cartesian axes, universal legends, and canvas margins. |
| `components/features/modals/visualizer/components/subcomponents/UniversalPaletteFontPanel.tsx` | UI Subcomponent | Standalone Universal Palette & Global Typography panel organized into two sub-tabs (Palette & Typography, Figure Title & Caption), standardizing 10 academic font family stacks, base proportional sizing, 36 publication-grade theme palettes, and main figure title/caption typography. |
| `components/features/modals/visualizer/components/subcomponents/UniversalLayoutMarginPanel.tsx` | UI Subcomponent | Universal Layout Margins and Coordinate Clearance panel controlling top/bottom/left/right chart margins, smart auto-fit, container padding, print-safe boundary guides, composite subfigure gutter and borders, and aspect ratios. |
| `components/features/modals/visualizer/components/subcomponents/UniversalDataLabelPanel.tsx` | UI Subcomponent | Universal Data Label and Metric Formatter panel controlling master label toggle, multi-line format templates, auto-adaptive/outside placement, distance offset, word wrap & collision truncation, max label width, auto-contrast color modes, and threshold filters. |
| `components/features/modals/visualizer/components/subcomponents/UniversalLegendConfigPanel.tsx` | Presentation Component | Universal legend configurator organized into 4 cohesive sub-tabs (Placement, Typography, Frame & Spacing, and CDS Hierarchy & Sync), featuring an extensive publication-grade typography suite (font family selection with 10 academic stacks, weights 400–800, upright/italic styles, font size, line height, max width, word wrap/truncation modes, text color with quick swatches), box frame styling (background, border, corner radius, padding), item geometry, and merged CDS Studio hierarchy & metric synchronizer with real-time discrepancy detection. |
| `components/features/modals/visualizer/components/subcomponents/StudioDataTab.tsx` | Presentation Tab | Encapsulates Centralized Data Source (CDS) variable assignments, interactive colon segment extraction buttons, dynamic Parent Scope Filter UI with suggestion pills, omnipresent custom group triggers, scientific metric modes, and extraction normalization protocols. |
| `components/features/modals/visualizer/components/subcomponents/StudioChartTypeTab.tsx` | Presentation Tab | Encapsulates the 18 scientific chart cards grid, format filter pills, and composite figure layout templates. |
| `scripts/test-cds-colon-lineage.mjs` | Test Suite Script | Comprehensive unit test suite verifying dynamic colon depth discovery, arbitrary segment extraction, token path preservation, flow conservation, bottom-anchoring of omitted baselines, typography spacing sanitization, upstream ancestral filtering, universal CDS Studio scope filtering, and cross-RQ lineage inheritance. |
| `scripts/test-stacked-bar-cds-studio.mjs` | Test Suite Script | Comprehensive unit test suite verifying Stacked Bar CDS Studio Context Intelligence, parent layer scoping, parent taxonomy prefixing, tag share calculation, and segment label formatting synchronization. |
| `scripts/test-distinct-palette-expansion.mjs` | Test Suite Script | Comprehensive unit test suite verifying palette-aware distinct color expansion, 100% achromatic fidelity for Academic Grayscale, monochromatic hue family preservation, and multi-hue categorical harmonic tonal derivation. |
| `scripts/test-stacked-bar-data-sync.mjs` | Test Suite Script | Comprehensive unit test suite verifying Stacked Bar Data Source & Format Synchronization, in-chart flow layer share denominator calculation, cohort adoption prevalence mirroring, ratio_percent formatting, and bidirectional UI alignment. |
| `scripts/test-visualizer-axis-sync.mjs` | Test Suite Script | Automated test suite verifying visualizer categorical axis parameter synchronization (`barYAxisWidth` <-> `axisLabelWidthY`, `barYAxisOverflow` <-> `axisLabelOverflowY`), slash delimiter preservation in word wrap, dynamic anti-collision title gaps, and grid left margin scaling. |
| `components/features/modals/visualizer/generators/axisConfigHelper.ts` | Library / Generator Helper | Calculates publication-grade ECharts Cartesian axis configs, foolproof scientific/engineering auto-newline wrapping with atomic token protection (b/g/n, TCP/IP, km/h, w/o, etc.), empirical category label span estimation, dynamic anti-collision title gaps, and universal grid margin clearances with safe title clamping. |
| `components/features/modals/visualizer/generators/matrixGenerators.ts` | Chart Strategy Generator | Generates Heatmap and Calendar publication charts with dynamic color gradients, cell sizing, pan/zoom offsets (fitOffsetX, fitOffsetY), container padding, and continuous visualMap scales. |
| `components/features/modals/visualizer/generators/kpiNetworkGenerators.ts` | Chart Strategy Generator | Generates Radar, Funnel, Graph, and Gauge publication charts with dynamic layout scaling, edge repulsion, node styling, and pan/zoom offsets (fitOffsetX, fitOffsetY). |
| `components/features/modals/visualizer/components/subcomponents/HorizontalBarConfigPanel.tsx` | UI Subcomponent | Dedicated horizontal bar configuration panel with duplicate mini-legend removed, focusing on bar dimensions, spacing, sorting, data labels, and benchmark reference lines. |
| `components/features/modals/visualizer/components/subcomponents/ClusteredBarConfigPanel.tsx` | UI Subcomponent | Dedicated clustered bar configuration panel with duplicate mini-legend removed, focusing on clustered group dimensions, cluster gaps, sorting, orientation, and data labels. |
| `components/features/modals/visualizer/components/subcomponents/SunburstLevelConfigPanel.tsx` | UI Subcomponent | Dedicated radial hierarchy sunburst configuration panel with duplicate mini-legend removed, focusing on multi-level radii, label orientations, auto-contrast, and anti-collision suppression. |
| `components/features/modals/visualizer/components/subcomponents/ChartConfigPanels.tsx` | UI Subcomponents Collection | Modular per-chart configuration panels for Vertical Bar, Line, Pie/Donut, Treemap, Heatmap, Boxplot, Scatter, Bubble, Graph, Gauge, and Calendar, cleanly deduplicated of redundant mini-legends and margins. |
| `components/features/modals/visualizer/components/subcomponents/TreemapConfigPanel.tsx` | UI Subcomponent | Standalone publication-grade Treemap configuration panel with 5 academic presets, partitioning algorithms (squarified, sliceAndDice, binary) with dynamic square ratio synchronization, multi-level hierarchy depth, interactive zoom-to-node drill-down with 1-click root reset, roam pan/zoom, native breadcrumb navigation, tile spacing, borders, reactive global geometry sliders with automatic level override clearing, color dynamics (mapping by index/value/id, alpha/saturation range fading), upper header banners, data label typography with word-wrapping boundaries, dual cohort denominator selection (grouped tree sum vs global review papers cohort), extended tile value formatting, and per-level depth tabs. |
| `scripts/test-visualizer-deep-audit.mjs` | Test Suite Script | Comprehensive automated test suite validating all 19 chart option generators, universal grid clearance on horizontal charts, boxplot axis orientation, calendar/funnel pan offsets, and absence of NaNs. |
| `scripts/test-visualizer-treemap-finetune.mjs` | Test Suite Script | Comprehensive unit test suite verifying Treemap parameter defaults, 5 academic presets, formatting & auto-contrast engine, dynamic levels array construction, native breadcrumb navigation, algorithm square ratio synchronization, global slider override clearing, layout presets, auto-contrast upper labels, safe recursive node value computation, and grouped vs global cohort denominator resolution. |
| `components/features/modals/visualizer/hooks/useChartCanvas.ts` | React Hook / Canvas Controller | Manages chart canvas viewport zoom, pan, full-screen expansion, export, and interactive hierarchy drill-down resets (`resetSlotDrillDown`) via ECharts instance dispatch. |
| `components/features/modals/visualizer/components/subcomponents/RadarConfigPanel.tsx` | UI Subcomponent | Standalone publication-grade Radar/Spider chart configuration panel with 5 academic 1-click presets, polar geometry (radius, start angle, center X/Y, polygon vs circular shapes), concentric split rings, radial spoke lines, shaded band themes, numeric scale ring labels and radial ticks, vertex indicator badge styling, multi-series spline polygon curves, and baseline fill colors. |
| `scripts/test-visualizer-radar-finetune.mjs` | Test Suite Script | Comprehensive unit test suite verifying Radar/Spider parameter defaults, 5 academic presets, 100% round-trip JSON serialization/deserialization, and ECharts polar grid and smooth series generation. |
| `src/lib/services/reference-syncer-service.ts` | Backend / Domain Service | Core Reference Syncer engine: parses `db/references.bib`, builds dual in-memory lookup indexes (DOIs, titles, author-year), matches SLR Magic Paper_IDs via SQLite `slr.db`, flags suspicious/unformatted in-text citations, and generates synchronized LaTeX code. |
| `src/app/api/reference-syncer/scan/route.ts` | REST Endpoint | Handles POST (scanning uploaded multi-file .tex contents against `references.bib` & `slr.db`) and GET (sample .tex files one-click loader from `tmp/`). |
| `src/app/api/reference-syncer/export/route.ts` | REST Endpoint | Handles POST (exporting synchronized .tex files with accepted/overridden citations and generating Markdown execution summary report). |
| `src/hooks/useReferenceSyncer.ts` | Custom Hook | Manages Reference Syncer state: uploaded files, scanning state, filter modes, search query, per-file overrides, diff modal state, and sequential/single file downloads. |
| `src/components/features/ReferenceSyncerView.tsx` | Feature View Container | Top-level view for Reference Syncer featuring multi-file drag-and-drop/browse ingestion, sample files quick loader, executive statistics HUD, file tabs, and mode switching. |
| `src/components/features/reference-syncer/SyncerStatsHud.tsx` | Presentation Component | Executive HUD cards displaying Total Scanned, Auto-Resolved, Valid In .bib, Ambiguous, Missing In .bib, and Suspicious/Flagged citation metrics with interactive filtering. |
| `src/components/features/reference-syncer/CitationInspectionTable.tsx` | Presentation Component | Interactive inspection table showing citation commands, original keys, target replacement keys, match types/confidence, expandable line context snippets, inline candidate selectors, custom override inputs, and ignore toggles. |
| `src/components/features/reference-syncer/SuspiciousFindingsPanel.tsx` | Presentation Component | Detailed issue auditor displaying detected unformatted in-text plain text citations (parenthetical and bracketed author-year references outside LaTeX macros), empty keys, and missing references with actionable suggestions. |
| `src/components/features/reference-syncer/TexDiffModal.tsx` | Modal Component | Side-by-side and unified diff inspector comparing original .tex content against synchronized output before export, with one-click copy to clipboard. |
| `src/components/features/reference-syncer/ExecutionSummaryPanel.tsx` | Presentation Component | Renders multi-file execution summary matrix (citations, replacements, preserved, flagged), export report buttons, and dedicated individual download cards/links for each synchronized .tex file. |
| `scripts/test-reference-syncer.mjs` | Test Suite Script | Automated unit test suite verifying `references.bib` parsing, sample `tmp/*.tex` reading, citation replacement symmetry, suspicious plain-text pattern detection, and sidebar/page routing. |
| `scripts/test-reference-syncer-e2e.mjs` | Test Suite Script | End-to-end test suite verifying resolution across AuthorYear patterns, ambiguous candidates, exact BibTeX key matches, and DOI lookups. |
| `src/lib/services/reference-syncer-types.ts` | Pure Domain Types & Helpers | Pure TypeScript domain types and client-safe utilities (string cleaning, author et al. formatting, text normalization) for Reference Syncer with zero Node.js/DB dependencies. |
| `src/app/api/reference-syncer/bib/route.ts` | REST Endpoint | Handles GET (status of `db/references.bib`, entry count, file size, mtime, loaded timestamp, forced reload via `?reload=true`) and POST (direct upload and atomic replacement of `references.bib` with syntax validation and backup). |
| `scripts/test-screening-ledger.mjs` | Test Suite Script | Comprehensive automated test suite verifying full screened corpus database querying (1,803 papers), stage-aware decision resolution, multi-stage screening audit records, and schema validator backwards compatibility. |
| `src/app/api/charts/route.ts` | REST Endpoint | Handles GET (list project-saved charts from SQLite `saved_charts`), POST (create or overwrite saved chart with strict project isolation and payload validation), and DELETE (scoped chart removal). |
| `components/features/modals/visualizer/components/subcomponents/CollapsibleToolbox.tsx` | UI Subcomponent | Adobe Premiere Pro-inspired collapsible toolbox container featuring expand/collapse disclosure chevrons, themed category icons, active summary badges, quick toggle switches, and parameter reset triggers. |
| `components/features/modals/visualizer/components/subcomponents/ChartLibraryModal.tsx` | Modal Component | Centralized FAIR-compliant Project Chart Library modal supporting 1-click database save, studio hydration, duplicate, delete, and legacy JSON preset file import with cross-tab BroadcastChannel synchronization. |
| `components/features/modals/visualizer/components/subcomponents/UniversalPaletteFontPanel.tsx` | UI Subcomponent | Universal Academic Color Palette & Typography Studio featuring 76 prestigious presets, instant search filtering, domain categorization pills (All, Elite Universities, Top Journals, Sequential, Dark Mode), swatch cards with accent indicators, and publication-ready typography controls. |
| `components/features/modals/visualizer/constants/themePalettes.ts` | Design Token Constants | Master repository of 76 prestigious academic color palettes covering top universities, flagship journals, sequential degradation ramps, and high-contrast dark themes with complete semantic tokens (colors, bg, text, subtext, border, accent, secondary, surface, gridLine, heatScale, isDark). |
| `components/features/modals/visualizer/utils/colorUtils.ts` | Library / Shading Utilities | Advanced color manipulation and token resolution engine providing linear/radial gradients, harmonic contrast calculations, dynamic heat scale ramps, and safe resolution of palette accent, secondary, surface, and gridLine tokens. |
| `components/features/insight-export/CohortPaperDetailsModal.tsx` | Modal Component | Comprehensive fullscreen paper inspection modal with split-screen layout: left pane hosting local sourced PDF (/api/pdf/serve) in IDE or cloud synced PDF link in viewer alongside bibliographic details; right pane hosting deep RQ taxonomy mapping with reasoning rules/verbatim quotes, QA appraisal scores, and raw JSON. |
| `src/components/features/modals/EcosystemModal.tsx` | Modal Component | Centralized interactive ecosystem architecture modal displaying all SLR Magic sub-modules, ports, capabilities, cross-module workflow steps, standardized file formats, and network copy links. |
| `../scripts/build-standalone-dist.mjs` | Monorepo Build Automation Script | Centralized cross-platform standalone distribution builder: stages versioned single-file HTML bundles, performs font tree-shaking, checks Go toolchain, compiles native binaries, and generates cryptographic SHA256SUMS.txt for Zenodo/OSF publishing. |
| `../launcher/main.go` | Native Micro-Server Launcher | Statically-linked cross-platform Go micro-server launcher embedding SPA assets, dynamic loopback port binding (127.0.0.1:0), CLI argument/drag-and-drop snapshot loading, 60s background heartbeat auto-termination, and navigator.sendBeacon shutdown handling. |
