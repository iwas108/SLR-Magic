...
| `scripts/test-umbrellanizer-export.mjs` | Test Automation | Standalone test suite verifying Umbrellanizer raw-to-umbrella taxonomy mapping export, justification inclusion, occurrence counting, and multi-project database isolation. |
...
| `scripts/test-cds-grouping-studio.mjs` | Test Automation | Standalone test suite verifying Centralized Data Source (CDS) variable discovery, multi-level colon hierarchy resolution (`ext:macro:`, `ext:sub:`, `ext:leaf:`, `ext:lv1:`, `ext:lv2:`, `ext:lv3:`), scoped cross-relation queries (`scopeFilter`), scientific empty/NOT_STATED sanitization, fast regrouping, and Data Query Safety Guard audits. |
| `scripts/test-cds-colon-lineage.mjs` | Test Automation | Standalone test suite verifying Centralized Data Source (CDS) colon depth detection ($1\dots N$), arbitrary segment extraction (`ext:lvX:*`, `ext:segment:X:*`), token-level parent path lineage tracing (preventing biological asset cloud cross-talk), strict flow conservation ($\sum \text{Inflow} = \text{Node Value} = \sum \text{Outflow}$), unstated baseline bottom-anchoring (`sankeyPinUnstatedToBottom`), and typographical label spacing sanitization. |
| `scripts/test-sankey-flow-barycenter.mjs` | Test Automation | Standalone test suite verifying Sankey proportional flow conservation ($\sum \text{Inflow} = \text{Node Width} = \sum \text{Outflow}$), Leong unstated routing to edge compilers, uniform column heights ($H_1 = H_2 = 5.0$), and destination-weighted (Barycenter) sorting. |
| `scripts/test-stacked-bar-cds-studio.mjs` | Test Automation | Standalone test suite verifying Stacked Bar CDS Studio Context Intelligence, layer-qualified stacks disambiguation across shared taxonomies (e.g. `[App] Security...` vs `[Net] Security...`), hover tooltip dual-metric formatting, precision synchronization, and grouped legend ordering. |
| `scripts/test-stacked-bar-data-sync.mjs` | Test Automation | Standalone test suite verifying Stacked Bar Data Source and Format synchronization, active in-chart flow calculations, cohort prevalence alignment, lock in-sync bidirectional controls, and legend/data label harmonizers. |
...
| `components/features/modals/visualizer/generators/` | Chart Strategy Generators | 7 specialized chart-family strategy modules (categorical bars, hierarchical trees/sunbursts, epistemic & temporal trend lines, proportions, correlations, matrix heatmaps, KPI & network graphs) with comprehensive multi-line/scrolling legend builder and master `buildChartOption()` dispatcher. |
...
| `src/components/features/reference-syncer/SyncedTextViewerModal.tsx` | Modal Component | Standalone modal for viewing and copying full synchronized LaTeX manuscript files with monospace styling, line numbers, multi-file tabs, line wrap toggle, live in-file search, and one-click clipboard copy. |
