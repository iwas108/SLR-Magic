...
| `components/features/modals/visualizer/components/subcomponents/HorizontalBarScatterConfigPanel.tsx` | Subcomponent | Dedicated fine-tuning panel for Dual-Axis Horizontal Bar & Scatter Overlay charts with controls for bar series, scatter marker symbols, border styles, and dual-axis ceilings/intervals. |
...
| `components/features/modals/visualizer/components/subcomponents/ChartConfigPanels.tsx` | Subcomponents | Centralized collection and exports of fine-tuning parameter inspector panels across all scientific chart types (including `BubbleConfigPanel`, `ScatterConfigPanel`, `RadarConfigPanel`, etc.). |
...
| `components/features/modals/visualizer/components/subcomponents/UniversalFineTunePanel.tsx` | UI Container | Universal fine-tune panel standardizing 6 coherent sections (Series Parameters, Axes & Scales, Legend & Keys, Typography & Palettes) across all 18 scientific charts. |
| `components/features/modals/visualizer/components/subcomponents/UniversalLegendConfigPanel.tsx` | Presentation Component | Universal legend geometry, offset distance, placement, font size, and key label formatter configurator. |
| `components/features/modals/visualizer/components/subcomponents/StudioDataTab.tsx` | Presentation Tab | Encapsulates variable assignments, omnipresent custom group triggers, scientific metric modes, and extraction normalization protocols. |
