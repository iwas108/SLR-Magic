...
export default function ManualScreeningDetailView({
  ...
  const getEcRules = () => {
    ...
    const ecField = selectedPaper.calibration_pool === 'pool_b' 
      ? activeProj.pool_b_ec_rules 
      : activeProj.ec_rules;
    
    if (!ecField) return [];
    try {
      return typeof ecField === 'string' ? JSON.parse(ecField) : ecField;
    } catch {
      return [];
    }
  };
  ...
  const isPoolC = selectedPaper.calibration_pool === 'pool_c';
      ...
      <div className="flex flex-col xl:flex-row gap-6 w-full pb-8">
        ...
        <div className="w-full xl:w-[480px] space-y-4">
          <div className="bg-card border border-border p-5 rounded-xl space-y-4 shadow-sm">
            ...
            {/* Option A Flow: Pool A/B decision criteria */}
            {!isPoolC ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      { val: 'QA_WAIT', label: 'QA Wait', activeColor: 'bg-amber-600 text-white border-amber-600 shadow-sm' }
                    ].map((item) => {
              ...
              /* Option B Flow: Pool C extraction rules and QA criteria forms */
              <div className="space-y-4">
                {renderStageSelector()}

                {/* Quality Appraisal rules */}
                {manualStage === 'scientist' && qaRules.length > 0 && (
                  <div className="space-y-3 pt-2">
                    <span className="block text-[9px] text-muted-foreground font-black uppercase tracking-wider border-b border-border pb-1">
                      Quality Appraisal Rules Check (Pool C)
                    </span>
                ...
                {/* Data Extraction Forms mapping */}
                {manualStage === 'miner' && extractionRules.length > 0 && (
                  <div className="space-y-3 pt-2">
                    <span className="block text-[9px] text-muted-foreground font-black uppercase tracking-wider border-b border-border pb-1">
                      Data Extraction Variables (Pool C)
                    </span>
                ...
                {/* Include/Exclude selection for Pool C */}
                <div>
                  <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1">Final Pool C Decision</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['INCLUDE', 'EXCLUDE'].map((d) => (
                      <button
                        key={d}
                        type="button"
                        ...
                        className={`py-2 rounded-lg text-xs font-bold transition-all border cursor-pointer ${
                          manualDecision === d
                            ? d === 'INCLUDE' ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm' : 'bg-rose-600 text-white border-rose-600 shadow-sm'
                            : 'bg-secondary border-border text-muted-foreground hover:bg-secondary/80'
                        }`}
                      >
                        {d}
