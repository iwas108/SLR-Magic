...
export function generateClusteredBarOption(ctx: ChartGeneratorContext): echarts.EChartsOption {
  ...
  const seriesObjects: any[] = seriesList.map((seriesKey, sIdx) => {
    ...
    const seriesLegendLabel = formatLegendLabel(seriesKey, { paperCount: seriesTotalCount }, effectiveLegendFormat);
  ...
  const effectiveLegendPos = legendPosMap[barLegendPosition] || { bottom: 15, left: 'center' };
