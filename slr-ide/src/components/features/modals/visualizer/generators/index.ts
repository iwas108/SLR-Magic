...
export function buildChartOption(params: BuildChartOptionParams): echarts.EChartsOption {
  ...
  const renderCategoryTooltip = (dataObj: any, fallbackName: string = '') => {
    ...
    const prevPct = dataObj.prevalencePct ?? (papers.length > 0 ? ((paperCount / papers.length) * 100) : 0);
    const tagPct = dataObj.tagPct ?? 0;

    const prevPctStr = formatPercentage(prevPct, params.decimalPrecision ?? 0, params.useTildeForCoarse ?? true);
    const tagPctStr = formatPercentage(tagPct, params.decimalPrecision ?? 0, params.useTildeForCoarse ?? true);
    const ratioStr = formatRatio(paperCount, papers.length, params.ratioStyle ?? 'n_over_N');

    let activeMetricHeader = '';
    if (metricMode === 'avg_citation') {
      const avgVal = typeof dataObj.value === 'number' ? (params.decimalPrecision === 0 ? Math.round(dataObj.value) : dataObj.value.toFixed(params.decimalPrecision ?? 1)) : dataObj.value;
      activeMetricHeader = `<span style="color:${palette.subtext};">Average Citations:</span> <strong>${avgVal} citations</strong><br/>`;
    } else if (metricMode === 'avg_qa') {
      const avgVal = typeof dataObj.value === 'number' ? (params.decimalPrecision === 0 ? Math.round(dataObj.value) : dataObj.value.toFixed(params.decimalPrecision ?? 1)) : dataObj.value;
      activeMetricHeader = `<span style="color:${palette.subtext};">Average QA Score:</span> <strong>${avgVal} / 8.00</strong><br/>`;
    } else if (metricMode === 'tag_share') {
      activeMetricHeader = `<span style="color:${palette.subtext};">Active Metric (Tag Share):</span> <strong>${tagPctStr}</strong><br/>`;
    }
    ...
    return `<div style="font-family:${font};font-size:12px;padding:2px;line-height:1.5;">
      ...
      ${activeMetricHeader}
      <span style="color:${palette.subtext};">Cohort Paper Count:</span> <strong>${ratioStr} papers</strong> <span style="color:${palette.subtext};font-size:11px;">(${prevPctStr} of cohort)</span><br/>
      <span style="color:${palette.subtext};">Share of Tags:</span> <strong>${tagPctStr}</strong> <span style="color:${palette.subtext};font-size:11px;">(${tagCount} tags)</span>
    </div>`;
  };
