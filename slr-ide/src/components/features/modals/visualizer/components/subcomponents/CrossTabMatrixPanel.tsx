...
export function CrossTabMatrixPanel() {
  const { props, config } = useVisualizerContext();
      ...
      crossTab.categories.forEach(cat => {
        crossTab.seriesList.forEach(s => {
          ...
          if (cell && cell.count > 0) {
            tsv += `${cat}\t${s}\t${cell.count}\t${cell.prevalencePct}%\t${cell.activeMetricVal}\n`;
          }
        });
      });
    }
    ...
    if (activeTab === 'matrix') {
      ...
      crossTab.categories.forEach(cat => {
        crossTab.seriesList.forEach(s => {
          const cell = crossTab.matrix[cat]?.[s];
          if (cell && cell.count > 0) {
            csv += `"${cat}","${s}",${cell.count},${cell.prevalencePct},${cell.activeMetricVal}\n`;
          }
        });
      });
    }
