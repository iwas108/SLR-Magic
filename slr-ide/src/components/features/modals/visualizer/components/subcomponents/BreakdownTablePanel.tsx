        ...
        <div className="flex items-center gap-2">
          ...
          {isTagShareMode ? (
            <div className={`px-2.5 py-1 rounded-full text-[11px] font-extrabold flex items-center gap-1.5 ${
              ...
              Tag Share Sum: {realDataBreakdown.activeSum}% {!isBalanced && '(!= 100%)'}
            </div>
          ) : (
          ...
          <tbody className="divide-y divide-border/40">
            {realDataBreakdown.rows.map((row, rowIdx) => (
              <tr key={`${row.parentName ? `${row.parentName}_` : ''}${row.name}_${rowIdx}`} className="hover:bg-secondary/40">
                ...
                <td className="py-1.5 px-2 text-right font-mono text-muted-foreground">N = {row.paperCount}</td>
                <td className="py-1.5 px-2 text-right font-mono text-muted-foreground">{row.paperPrevalencePct.toFixed(2)}%</td>
                <td className="py-1.5 px-2 text-right font-mono text-muted-foreground">{row.tagCount}</td>
                <td className="py-1.5 px-2 text-right font-mono text-muted-foreground">{row.tagSharePct.toFixed(2)}%</td>
                <td className="py-1.5 px-2 text-right font-mono font-bold text-foreground">
                  {enableManualOverrides ? (
                    ...
                    <span>
                      {metricMode === 'count' ? `N = ${row.count}` : `${row.activeVal.toFixed(2)}%`}
                    </span>
                  )}
                </td>
              </tr>
