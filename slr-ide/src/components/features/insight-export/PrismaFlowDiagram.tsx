...
import PrismaConfigModal, { PrismaConfig } from '../modals/PrismaConfigModal';
...
export default function PrismaFlowDiagram({ projectId, showToast }: PrismaFlowDiagramProps) {
  ...
  useEffect(() => {
    if (!projectId) return;
    const saved = localStorage.getItem(`slr_prisma_config_${projectId}`);
    if (saved) {
      try {
  ...
  const handleConfigChange = (newConfig: PrismaConfig) => {
    setConfig(newConfig);
    if (projectId) {
      localStorage.setItem(`slr_prisma_config_${projectId}`, JSON.stringify(newConfig));
    }
  };
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        showToast(err.message || 'Error fetching PRISMA data', 'error');
      } finally {
        setLoading(false);
      }
    }
