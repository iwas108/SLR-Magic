...
import { Calendar, ShieldCheck, Sparkles, BookOpen, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ProjectActivityLog({ activeProject }: ProjectActivityLogProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-md p-6 space-y-4 w-full">
      <div 
        className="flex items-center justify-between border-b border-border pb-3 cursor-pointer group"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <div className="space-y-1">
          <h3 className="font-bold text-xs text-foreground uppercase tracking-wider flex items-center gap-2 group-hover:text-primary transition-colors">
      ...
      {isExpanded && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-1 text-xs animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border text-[10px] text-muted-foreground animate-in slide-in-from-top-2 duration-300 delay-75">
