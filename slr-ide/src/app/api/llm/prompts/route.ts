  ...
  try {
    ...
    if (id) {
      ...
      const stmt = db.prepare(`
        ...
        WHERE id = ?
      `);
      stmt.run(name, description, prompt_type || null, system_prompt, user_prompt_template, response_schema || null, llm_config || '{}', is_active ? 1 : 0, now, id);
    } else {
      // Insert
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      stmt.run(newId, project_id || null, name, description, prompt_type || null, system_prompt, user_prompt_template, response_schema || null, llm_config || '{}', is_active ? 1 : 0, now, now);
    }
