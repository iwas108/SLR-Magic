...
export async function POST(request: Request) {
  try {
    ...
    if (calibration_pool && ['pool_a', 'pool_b', 'pool_c'].includes(calibration_pool)) {
      ...
      const existing = db.prepare("SELECT 1 FROM calibration_papers WHERE Paper_ID = ? AND Project_ID = ?").get(paperId, targetProjectId);
      if (!existing) {
        ...
        `).run(targetProjectId, paperId, targetProjectId);
      }
      ...
      db.prepare("DELETE FROM calibration_papers WHERE Paper_ID = ? AND Project_ID = ?").run(paperId, targetProjectId);
    }
