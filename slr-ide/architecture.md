...
*   **Interactive Adjudication Explorer & Inspection Modal**: The pre-calibration view and scientific rigor suite embed interactive adjudication explorers allowing reviewers and auditors to inspect paired decisions across Pool A, Pool B, and Pool C, switch between paired cohorts and cryptographically signed commit ledgers, filter by conflicts vs all papers, and launch dual-pane inspection modals (`AdjudicationInspectionModal.tsx`) with side-by-side criteria-level rubrics and source evidence quotes.
