...
const Initializer = (function() {
  ...
  function runInitialization() {
    ...
    // 3. Clear/Create all 9 sheets with hardcoded required columns
    
    // -- 00_Raw_Harvest: Base + Status
    const sheetRaw = clearOrCreateSheet(ss, "00_Raw_Harvest");
    setupSheet(sheetRaw, [...BASE_HEADERS, 'Status']);

    // -- 01_Fast_Filter: Base + EVAL + Status
    const sheetFast = clearOrCreateSheet(ss, "01_Fast_Filter");
    setupSheet(sheetFast, [...BASE_HEADERS, ...EVAL_HEADERS, 'Status']);

    // -- 02_Gatekeeper: Base + EVAL + Status
    const sheetGate = clearOrCreateSheet(ss, "02_Gatekeeper");
    setupSheet(sheetGate, [...BASE_HEADERS, ...EVAL_HEADERS, 'Status']);

    // -- 03_Scientist: Base + EVAL + Status
    const sheetSci = clearOrCreateSheet(ss, "03_Scientist");
    setupSheet(sheetSci, [...BASE_HEADERS, ...EVAL_HEADERS, 'Status']);

    // -- 04_Miner: Base + Status
    const sheetMiner = clearOrCreateSheet(ss, "04_Miner");
    setupSheet(sheetMiner, [...BASE_HEADERS, 'Status']);
    ...
    const alertMessage = "Workspace Initialized Successfully!\n\n" +
      "All 9 sheets have been generated with required system columns.\n\n" +
