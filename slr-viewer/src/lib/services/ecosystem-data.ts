export interface EcosystemModule {
  id: 'slr-ide' | 'inter-rater' | 'slr-viewer' | 'worker-server';
  name: string;
  shortName: string;
  role: string;
  tagline: string;
  ...
  defaultPort?: number;
  badgeText: string;
  badgeVariant: 'primary' | 'secondary' | 'emerald' | 'amber' | 'purple' | 'sky';
  gradient: string;
  iconName: string;
...
export interface EcosystemWorkflowStep {
  step: number;
  name: string;
  fromModuleId: 'external' | 'slr-ide' | 'inter-rater' | 'slr-viewer';
  toModuleId: 'slr-ide' | 'inter-rater' | 'slr-viewer' | 'external';
  exchangeType: string;
  fileFormat: string;
  description: string;
...
export interface EcosystemFileSpec {
  extension: string;
  fullName: string;
...
export const ECOSYSTEM_WORKFLOW_STEPS: EcosystemWorkflowStep[] = [
  {
    step: 1,
...
export const ECOSYSTEM_FILE_SPECS: EcosystemFileSpec[] = [
  {
    extension: '.slr',
 ...
 */
export function resolveModuleUrl(defaultPort?: number, moduleId?: string, fallbackPath?: string): string {
