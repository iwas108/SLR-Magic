 ...
 */
export function renderPoolCReviewerSummary(qaScoresInput: string | Record<string, any> | undefined | null, qaRules: QaRule[]): string {
  try {
    const qaScores = typeof qaScoresInput === 'object' && qaScoresInput !== null
      ? qaScoresInput
      : JSON.parse(String(qaScoresInput || '{}'));
    let hasFatal = false;
    let score = 0;
