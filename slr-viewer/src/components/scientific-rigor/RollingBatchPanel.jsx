...
export default function RollingBatchPanel({ rollingBatchQC }) {
  if (!rollingBatchQC) {
    return (
  ...
  const completedBatches = batches.filter(b => b.status === 'PASSED' || b.status === 'complete');
              ...
              <tbody className="divide-y divide-border/40">
                {batches.map((batch) => {
                  const indStats = individual_batch_stats.find(
                    (s) => s.batchId === batch.id || s.batchNumber === batch.batch_number
                  )?.stats;
