...
export default function FinalCohortPanel() {
  ...
  const umbrellanizerMap: any = useMemo(() => activeSession?.rawData?.final_cohort?.umbrellanizer_mappings || {}, [activeSession]);
  const projectData: any = useMemo(() => activeSession?.rawData?.project || {}, [activeSession]);
  ...
  const getColWidth = useCallback((key: string, isQa = false, isExt = false): number => {
  ...
  const parseExtractedTraces = useCallback((paper: any) => {
    ...
    try {
      ...
      Object.keys(extObj).forEach(key => {
        if (key.startsWith('_') || key === 'logic_trace' || key === 'logicTrace' || key === '_scientist_logic_trace') return;
    ...
    return { mapping: {} as Record<string, string>, evidence: {} as Record<string, string> };
  }, []);
                    ...
                    return (
                      ...
                      >
                        ...
                        {filterOptions.qaKeys.map((qaKey) => {
                          ...
                          const paperPdfLink = p.PDF_Link || undefined;
