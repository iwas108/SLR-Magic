...
export default function VisualizerModal({
  ...
  const [chartType, setChartType] = useState('bar_vertical');

  // Step 2 State: Data Mapping Parameters
  const [primaryField, setPrimaryField] = useState('Year');
  const [secondaryField, setSecondaryField] = useState('Import_Source');
  const [metricMode, setMetricMode] = useState('count');
  
  // Dynamic Multi-Level Flow Fields (for Sankey, Sunburst, Treemap)
  const [sankeyFields, setSankeyFields] = useState(['Year', 'Import_Source', 'Local_PDF_Status']);
  const [sankeyLabelPositions, setSankeyLabelPositions] = useState({});
  const [sankeyMaxNodes, setSankeyMaxNodes] = useState({});

  // Data Limiting Parameters
  const [limitCategories, setLimitCategories] = useState(false);
  const [maxCategoriesCount, setMaxCategoriesCount] = useState(10);

  // Numerical Field Selectors (for Scatter, Bubble, Boxplot)
  const [numFieldX, setNumFieldX] = useState('Overall_QA');
  const [numFieldY, setNumFieldY] = useState('citation_count');
  const [numFieldSize, setNumFieldSize] = useState('Year');

  // Cell Value Options
  const [useUmbrellanizer, setUseUmbrellanizer] = useState(true);
  const [splitMultiValues, setSplitMultiValues] = useState(true);
  const [excludeEmpty, setExcludeEmpty] = useState(true);

  // Step 3 State: Style Customization
  const [chartTitle, setChartTitle] = useState('Cohort Distribution');
  const [chartSubtitle, setChartSubtitle] = useState('Systematic Literature Review Paper Analysis');
  const [themePreset, setThemePreset] = useState('ieee_blue');
  ...
  const [donutRatio, setDonutRatio] = useState(50); // Pie
  const [smoothLine, setSmoothLine] = useState(true); // Line
  const [sankeyNodeWidth, setSankeyNodeWidth] = useState(20); // Sankey
  const [sankeyNodeGap, setSankeyNodeGap] = useState(18); // Sankey
  const [sankeyLeftPadding, setSankeyLeftPadding] = useState(8); // Sankey left edge %
  const [sankeyRightPadding, setSankeyRightPadding] = useState(20); // Sankey right edge %
  const [bubbleScale, setBubbleScale] = useState(1.2); // Bubble/Scatter
  const [gaugeMaxScale, setGaugeMaxScale] = useState(100); // Gauge dial max target
  ...
  const resolveValue = useCallback((val, key) => {
  ...
  const getFieldValue = useCallback((paper, fieldKey) => {
    if (!fieldKey) return [];
    ...
    if (fieldKey.startsWith('ext:')) {
      ...
      try {
        const parsed = typeof qaStr === 'object' ? qaStr : JSON.parse(qaStr);
        const qaObj = parsed.qa_scores || parsed;
        let score = 0;
        Object.values(qaObj).forEach((v) => {
  ...
  const getNumericalValue = useCallback((paper, numKey) => {
  ...
  const computeMetricValue = useCallback((groupPapers) => {
  ...
  const limitCategoryMap = useCallback((countsMap) => {
  ...
  const generateChartOption = useCallback(() => {
    const palette = THEME_PALETTES[themePreset] || THEME_PALETTES.dark_modern;
    ...
    const baseTitle = {
      ...
      left: 'center',
      top: 15,
      textStyle: { fontFamily: font, fontSize: fontSize + 4, fontWeight: 'bold', color: palette.text },
    ...
    if (chartType === 'scatter') {
      ...
      return {
        ...
        title: baseTitle,
        legend: baseLegend,
        tooltip: { ...baseTooltip, formatter: (p) => `<strong>${p.data[2]}</strong><br/>${numFieldX}: ${p.data[0]}<br/>${numFieldY}: ${p.data[1]}` },
    ...
    if (chartType === 'bubble') {
      ...
      return {
        ...
        title: baseTitle,
        legend: baseLegend,
        tooltip: { ...baseTooltip, formatter: (p) => `<strong>${p.data[3]}</strong><br/>${numFieldX}: ${p.data[0]}<br/>${numFieldY}: ${p.data[1]}<br/>${numFieldSize}: ${p.data[2]}` },
        grid: { left: '10%', right: '10%', top: showLegend ? 110 : 80, bottom: '15%', containLabel: true },
        ...
        yAxis: { type: 'value', name: numFieldY, nameLocation: 'middle', nameGap: 35, axisLabel: { fontFamily: font, fontSize: fontSize - 1, color: palette.text }, splitLine: { lineStyle: { color: palette.border, type: 'dashed' } } },
        series: [{ type: 'scatter', symbolSize: (d) => Math.max(6, Math.round(d[2] * 2 * bubbleScale)), data: bubbleData, itemStyle: { opacity: 0.75 } }]
      };
    }
    ...
    if (chartType === 'treemap') {
      const allowedLevelSets = sankeyFields.map((fieldKey, idx) => {
        ...
        return new Set(sorted.slice(0, limitCount - 1).map(e => e[0]));
      });

      const buildTree = (papersList, levelIdx) => {
    ...
    if (chartType === 'heatmap') {
      ...
      const catXSet = new Set();
      const catYSet = new Set();
      ...
      return {
        backgroundColor: palette.bg,
        title: baseTitle,
        tooltip: { ...baseTooltip, formatter: (p) => `${xData[p.data[0]]} × ${yData[p.data[1]]}: ${p.data[2]} papers` },
    ...
    if (chartType === 'sankey') {
      ...
      const nodes = Array.from(nodesSet).map(n => {
        ...
        const defaultPos = levelNum === sankeyFields.length ? 'left' : 'right';
    ...
    if (chartType === 'boxplot') {
      ...
      return {
        ...
        title: baseTitle,
        legend: baseLegend,
        tooltip: { ...baseTooltip, trigger: 'item', formatter: (params) => `<strong>${params.name}</strong><br/>Min: ${params.data[1]}<br/>Q1: ${params.data[2]}<br/>Median: ${params.data[3]}<br/>Q3: ${params.data[4]}<br/>Max: ${params.data[5]}` },
        grid: { left: '10%', right: '10%', top: showLegend ? 110 : 80, bottom: '15%', containLabel: true },
        xAxis: { type: 'category', data: categories, axisLabel: { fontFamily: font, fontSize: fontSize - 1, color: palette.text, rotate: labelRotation } },
        yAxis: { type: 'value', name: numFieldY, nameLocation: 'middle', nameGap: 35, axisLabel: { fontFamily: font, fontSize: fontSize - 1, color: palette.text }, splitLine: { lineStyle: { color: palette.border, type: 'dashed' } } },
        series: [{
    ...
    if (chartType === 'graph') {
      ...
      return {
        ...
        tooltip: { ...baseTooltip, formatter: (p) => p.dataType === 'edge' ? `${p.data.source} → ${p.data.target}: ${p.data.value} papers` : p.name },
        series: [{
          type: 'graph',
          layout: 'force',
    ...
    if (chartType === 'calendar') {
      ...
      return {
        backgroundColor: palette.bg,
        title: baseTitle,
        tooltip: { ...baseTooltip, formatter: (p) => `${p.data[0]}: ${p.data[1]} papers ingested` },
        visualMap: {
          min: 0,
          max: maxVal,
          type: 'continuous',
          ...
          <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center max-w-5xl mx-auto w-full space-y-8">
            ...
            <div className="w-full flex justify-end pt-4">
              <button
                ...
                className="px-6 py-2.5 bg-primary text-primary-foreground hover:bg-primary/95 rounded-xl text-xs font-bold shadow-md flex items-center gap-2 transition-all hover:scale-105 active:scale-95 cursor-pointer"
              >
          ...
          <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center justify-center max-w-3xl mx-auto w-full space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full bg-secondary border border-border rounded-xl px-3 py-2 text-xs font-bold text-foreground focus:outline-none focus:border-primary cursor-pointer"
                    >
                  ...
                  <div className="flex items-center justify-between p-3 bg-primary/5 border border-primary/20 rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="w-7 h-7 rounded-lg bg-secondary border border-border font-bold text-xs flex items-center justify-center disabled:opacity-40 hover:bg-secondary/80 cursor-pointer"
                      >
                        -
                      </button>
                      <button
                    ...
                    {sankeyFields.map((fieldVal, idx) => (
                      <div key={idx} className="p-3 bg-secondary/30 border border-border rounded-xl space-y-2">
                        ...
                        <select
                          ...
                          className="w-full bg-card border border-border rounded-lg px-2.5 py-1.5 text-xs font-bold text-foreground focus:outline-none focus:border-primary cursor-pointer"
                        >
                          {availableFields.map(f => (
                            <option key={f} value={f}>{f.startsWith('ext:') ? `Extracted: ${f.substring(4)}` : f}</option>
                          ))}
            ...
            <div className="w-full flex justify-between pt-2">
              <button
                ...
                className="px-5 py-2.5 bg-secondary text-foreground hover:bg-secondary/80 rounded-xl text-xs font-bold flex items-center gap-2 transition-colors border border-border cursor-pointer"
              >
                ...
                <div className="space-y-1.5">
                  ...
                  {showLegend && (
                    <select
                      ...
                      className="w-full bg-card border border-border rounded-lg px-2 py-1 text-xs text-foreground font-semibold cursor-pointer"
                    >
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 bg-background flex flex-col p-6 overflow-hidden relative border-r border-border">
              ...
              <div className="h-12 border-b border-border bg-card/60 rounded-t-2xl px-4 flex items-center justify-between shrink-0 mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setCurrentStep(3)}
                    className="px-3 py-1.5 bg-secondary text-foreground hover:bg-secondary/80 rounded-lg text-xs font-bold flex items-center gap-1.5 border border-border cursor-pointer"
                  >
            ...
            <div className="w-[340px] bg-secondary/10 flex flex-col p-6 space-y-6 shrink-0 overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`p-3 rounded-xl border text-center font-bold text-xs transition-colors cursor-pointer ${exportFormat === 'png' ? 'bg-primary/10 border-primary text-primary shadow-sm' : 'bg-card border-border hover:bg-secondary/40 text-foreground'}`}
                    >
                      PNG (Raster)
                    </button>
                    <button
                      onClick={() => setExportFormat('svg')}
                      className={`p-3 rounded-xl border text-center font-bold text-xs transition-colors cursor-pointer ${exportFormat === 'svg' ? 'bg-primary/10 border-primary text-primary shadow-sm' : 'bg-card border-border hover:bg-secondary/40 text-foreground'}`}
                    >
                      SVG (Vector)
                    </button>
                  </div>
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                      {[1, 2, 3, 4].map(s => (
                        <button
                          ...
                          className={`py-2 rounded-xl border font-extrabold text-xs transition-colors cursor-pointer ${exportScale === s ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-foreground'}`}
                        >
                          {s}x
                        </button>
                      ))}
                ...
                <div className="pt-4">
                  <button
                    ...
                    className="w-full py-3 bg-primary text-primary-foreground hover:bg-primary/95 rounded-xl text-xs font-extrabold shadow-lg flex items-center justify-center gap-2 transition-all hover:scale-105 active:scale-95 cursor-pointer"
                  >
              ...
              <div className="pt-2">
                <button
                  onClick={() => setCurrentStep(3)}
                  className="w-full py-2 bg-secondary text-foreground hover:bg-secondary/80 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-colors border border-border cursor-pointer"
                >
                  <ArrowLeft className="w-4 h-4" />
                  Back to Customize
                </button>
