...
*   **Form Input Preservation (Rehydration Guard)**: When re-hydrating active states, you **MUST** guard editable form inputs from being reset or overwritten. Use a mutable `useRef` (e.g., `lastLoadedPaperRef`, `lastLoadedDiscrepancyRef`, `lastLoadedProjectRef`) to cache the database state of the currently loaded object. Only reset or overwrite local user-input states if the object ID changes (indicating the user clicked a different paper) or if the form-related input states in the database itself changed from another session (multi-tab sync). This preserves local unsaved typed inputs when background tasks update non-form attributes (like `local_pdf_path`).
*   **Active State Downgrade Prevention Guard**: When re-hydrating an active selection from a background list in a `useEffect` (e.g. `papers.find(p => p.Paper_ID === selectedPaper.Paper_ID)`), you **MUST NEVER** overwrite an active item that has a newly acquired or updated state (such as `Local_PDF_Status !== 'MISSING'` and `Local_PDF_Path` present) if the background list record still reflects stale/missing data. Always guard active state sync against list downgrades.
*   **Stream Completion Symmetry**: Any SSE / NDJSON stream or background process that updates a paper record (such as single PDF acquisition, batch scanning, or screening adjudication) **MUST**:
    1. Capture the target paper ID via a mutable ref (`currentRunningPaperIdRef`) to avoid stale closure bugs.
    2. Re-fetch the fresh paper record by ID upon completion.
    3. Update both the active selection/modal state and the background array.
    4. Call `broadcastSync('SYNC_PAPERS')` and `broadcastSync('SYNC_PROJECTS')`.
*   **Universal Sync Channel Subscription**: Any hook or view that manages paper datasets, calibration pools, or screening records (`usePapers`, `useManualScreening`, `useCalibration`, `usePromptStaging`, etc.) **MUST** subscribe to `subscribeSyncChannel` and trigger appropriate data reloaders upon receiving `SYNC_PAPERS` or `SYNC_PROJECTS`.
...
*   **Absolute Multi-Project Isolation**: Every SQL query (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `JOIN`, or subquery) operating on project-tied database tables (`papers`, `reviewer_decisions`, `calibration_commit_ledger`, `calibration_papers`, `manual_audit_log`, `llm_audit_log`, `duplicate_pairs`, `rolling_batches`, `rolling_batch_papers`, `rolling_batch_reviewer_decisions`, `rolling_batch_commit_ledger`, `umbrellanizer_results`) **MUST** explicitly include and enforce `(Project_ID = ? OR CAST(Project_ID AS TEXT) = CAST(? AS TEXT))` (or `(project_id = ? OR CAST(project_id AS TEXT) = CAST(? AS TEXT))`).
*   **Explicit Project ID Parameter Precedence**: All API endpoints and Python background processes **MUST** accept and prioritize an explicit `projectId` (via query parameter `searchParams.get('projectId')`, JSON request body `body.projectId`, or CLI argument `--project`) over global configuration fallback (`getConfig('ACTIVE_PROJECT_ID')`).
*   **Safe Project ID Discovery Fallback**: If `projectId` is omitted in a single-item request (e.g., viewing screening records or single PDF acquisition), the endpoint **MUST** discover the target item's `Project_ID` from the database `papers` table rather than assuming the currently active project config matches the item.
*   **Mandatory Type-Agnostic Project ID Matching**: Project IDs in SQLite may exist as strings (e.g. `'proj-1'`) or numbers. All SQL queries operating on project-tied tables **MUST** use the type-agnostic clause:
    `(Project_ID = ? OR CAST(Project_ID AS TEXT) = CAST(? AS TEXT))` or `(project_id = ? OR CAST(project_id AS TEXT) = CAST(? AS TEXT))`
...
*   **Cascading Project Deletion**: When deleting a project, all associated records in all 11 project-tied tables **MUST** be deleted inside an atomic SQLite transaction filtered strictly by `(Project_ID = ? OR CAST(Project_ID AS TEXT) = CAST(? AS TEXT))`.
*   **Vector Search & Scraper Allowlisting**: FAISS vector search engines (`vector_worker.py`, `semantic_search.py`), PDF integrity verification tools (`verify_pdfs.py`), and scraper scripts (`scrape_pdfs.py`, `match_cache.py`, `find_traps.py`, `map_publisher.py`) **MUST** restrict candidate allowlists and file updates strictly to the target `project_id` using type-agnostic matching.
*   **Remote Worker Result Scoping**: Callbacks handling PDF download or scrape results (`remote-worker/result/route.ts`) **MUST** resolve the target paper's `Project_ID` and include `AND (Project_ID = ? OR CAST(Project_ID AS TEXT) = CAST(? AS TEXT))` on all database updates to prevent cross-project state corruption.
