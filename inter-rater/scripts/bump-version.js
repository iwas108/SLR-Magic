...
function computeSourceHash() {
  ...
  const sourceTargets = [
    ...
    path.join(moduleDir, 'vite.config.js'),
  ];
...
try {
  ...
  if (hasChanges) {
    ...
    console.log(`[inter-rater] Source changes detected. Version bumped from ${currentVersion} to ${pkgData.version}`);
  } else {
    console.log(`[inter-rater] No source code changes detected. Preserving version v${currentVersion}`);
  }
} catch (err) {
  console.error('[inter-rater] Failed in smart versioning check:', err);
}
